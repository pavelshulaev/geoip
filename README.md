...
Местоположение определяется по 3м службам:
* ipgeobase.ru (IpGeoBase);
* sypexgeo.net (Sypex);
* freegeoip.net (FreeGeoIp).

В обычном режиме решение предоставляет данные из первой службы, корректно вернувшей данные. В случае необходимости, можно явно указать еобходимую службу в 3м параметре `Location::getInstance()`.
...
Третьим параметром можно передать название предпочтительной службы. На данный момент доступны `'IpGeoBase'`, `'Sypex'` и `'FreeGeoIp'`. Если название не передано, данные будут получены из первой, корректно их получившей. Службы вызываются в порядке, указанном выше.
