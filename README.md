...
В обычном режиме решение предоставляет данные из первой службы, корректно вернувшей данные. Службы вызываются в указанном порядке. В случае необходимости, можно явно указать необходимую службу в 3м параметре `Location::getInstance()`.
